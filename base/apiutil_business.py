...
class RequestBase(object):
    ...
    def replace_load(self, data):
        """yaml数据替换解析"""
        # 为了能方便地进行字符串查找和替换，函数首先需要确保处理的数据是字符串格式
        str_data = data
        ...
        for i in range(str_data.count('${')):
            if '${' in str_data and '}' in str_data:
                ...
                # 动态地从 DebugTalk 类中找到名为 func_name 的方法，然后将 func_params 字符串分割成多个参数并传入该方法
                # 最后执行这个方法并将返回值赋给 extract_data 变量
                extract_data = getattr(DebugTalk(), func_name)(*func_params.split(',') if func_params else "")
                # 如果动态函数返回的结果是一个列表，那么就把它转换成一个逗号分隔的字符串。否则，保持原样
